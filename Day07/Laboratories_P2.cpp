...
{
private:
    int paths(vs &arr, int origin, int index, vvi &dp)
    {
        if (dp[index][origin] >= 0)
            return dp[index][origin];

        if (index == arr.size() - 1)
        {
            if (arr[index][origin] == '^')
                return dp[index][origin] = 2;
            return dp[index][origin] = 1;
        }

        if (arr[index][origin] == '^')
            return dp[index][origin] = paths(arr, origin + 1, index + 1, dp) + paths(arr, origin - 1, index + 1, dp);
        return dp[index][origin] = paths(arr, origin, index + 1, dp);
    }
...
public:
    ...
    void solve(istream &in = cin, ostream &out = cout)
    {
        ...
        vvi dp(arr.size(), vector<int>(n, -1));

        cout << paths(arr, origin, 0, dp) << endl;
    }
};
